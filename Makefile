...
docker-logs:               ## Stop the runnig container
	@docker logs -f `docker ps -q --filter="ancestor=$(CONTAINER_NAME)"` || (echo "Container not running: [ $(CONTAINER_NAME) ]"; exit 0)
...
docker-run-daemon:         ## Run the docker container.
	$(MAKE) docker-build
	$(MAKE) docker-stop
	@docker run -d --env HUBOT_SLACK_TOKEN=${AMIBOT_SLACK_TOKEN} $(CONTAINER_NAME)
